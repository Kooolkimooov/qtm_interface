...
class Controller:
    ...
    def __init__(self) -> None:
        ...
        self.waypoints = np.array([[1, 1], [1.4, -1], [0, 0], [-0.8, -0.2], [1, -0.5], [-0.4, -0.4]])
        ...
        self.theta_w = np.arctan(np.abs(self.tb_center[1] - self.waypoint[1]) / np.abs(self.tb_center[0] - self.waypoint[0]))
    ...
    def acquistion(self,pose):
        if (pose.position.x != np.nan or pose.position.y != np.nan or pose.orientation.z != np.nan):
    ...
    def linear_tb(self,waypoint, TB_center, theta_tb, MaxLinearSpeed, MaxLinearAccel, linearSpeed, epsilon):
        ...
        dist_stop = linearSpeed**2 / (2 * MaxLinearAccel)  # Voir démonstration
        linearSpeed = 0
        bool_val = False
