from numpy import array, concatenate, cos, cross, diag, exp, eye, ndarray, sin, tan, zeros, set_printoptions
...
class Bluerov( Simulator ):
  ...
  def __call__( self, state: ndarray, actuation: ndarray, perturbation: ndarray = None ) -> ndarray:
    ...
    self.buoyancy[ 2 ] = rho_eau * G * self.volume * (1. - .5 / (1 + exp(
  ...
  @staticmethod
  def actuation_from_ros( ros_actuation ) -> ndarray:
    actuation = zeros( (Bluerov.actuation_size,) )

    actuation[ 0 ] = 4 * 3 * G * 0.7071067812 * (ros_actuation.channels[ 4 ] - 1500) / 400
    actuation[ 1 ] = 4 * 3 * G * 0.7071067812 * (ros_actuation.channels[ 5 ] - 1500) / 400
    actuation[ 2 ] = 4 * 3 * G * (ros_actuation.channels[ 2 ] - 1500) / 400
    actuation[ 3 ] = 4 * 3 * G * .1 * (ros_actuation.channels[ 1 ] - 1500) / 400
    actuation[ 4 ] = 4 * 3 * G * .1 * (ros_actuation.channels[ 0 ] - 1500) / 400
    actuation[ 5 ] = 4 * 3 * G * .1 * (ros_actuation.channels[ 3 ] - 1500) / 400
